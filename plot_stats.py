...
#########################
N = n_subjects = 50       # repetitions of simulation to average over
...
def plot_distance(df, metric="task_euclid", ymeasure="regret"):
    ...
    m_uvfa = model_uvfa.params[f"{metric}"]
    b_sfgpi = model_sfgpi.params["Intercept"]
    m_sfgpi = model_sfgpi.params[f"{metric}"]

    # correlation
    r_uvfa = np.corrcoef(df_uvfa[f"{metric}"], df_uvfa[ymeasure])[0,1]
    r_sfgpi = np.corrcoef(df_sfgpi[f"{metric}"], df_sfgpi[ymeasure])[0,1]
    ...
    plt.figure()
    sb.scatterplot(x=f"{metric}", y=ymeasure, hue="algo", alpha=.5,
...
def plot_changes(df, change="task", ymeasure="regret"):
    ...
    # discrete metric (change or not)
    if ymeasure != "correct": # has to be continous, "correct" is binary
...
if __name__ == "__main__":
    ...
    #plot_first_n_trials(df, offset=0, change="task", ymeasure="correct")

    #plot_distance(df, metric="task_angle", ymeasure="correct")
    #plot_distance(df, metric="max_value_diff", ymeasure="correct")

    #plot_changes(df, change="task", ymeasure="regret")

    plot_blocks(df, change="feature", ymeasure="correct")
