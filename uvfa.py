...
class SFGPI():
    ...
    def __init__(self, input_size, output_size, feature_dim,
    ...
    def train(self, step):
        ...
        a_batch       = torch.vstack(batch.action)
        phi_batch     = torch.vstack(batch.reward)
        s_next_batch  = torch.vstack(batch.next_state)

        # compute TD error (psi vs phi, not Q vs r!)
        # get prediction for next state from target net for better stability
        # all vars have size: BATCH_SIZE x n_features
        psi_a    = self.policy_net.forward(s_batch).gather(1, a_batch)
        psi_next = self.target_net(s_next_batch).max(1).squeeze() # max over a
        delta    = phi_batch + self.GAMMA * phi_next - phi_a
